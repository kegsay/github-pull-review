...
var TriggerMixin = require("../logic/triggers").TriggerMixin([
    triggers.PullRequestTrigger.TYPE, "file_diffs", "line_comments"
]);
...
module.exports = React.createClass({displayName: 'ActionsPage',
    ...
    onReceiveAction: function(action, data) {
        ...
        if (action === "line_comments" && this.state.actions) {
            ...
            // is done horribly currently by ticking to make sure the TriggerMixin
    ...
    _loadDiffs: function(props) {
        ...
        if (pullRequest) {
            ...
            this.setState({
                [triggers.PullRequestTrigger.TYPE]: prTrigger
            });
        }
    ...
    onSubmitLineReply: function(action, text) {
        return this.props.controller.postReplyLineComment(
            this.state[triggers.PullRequestTrigger.TYPE].pr, text, action.getHeadComment()
        );
    },
    ...
    render: function() {
        var self = this;
        var pr = this.state[triggers.PullRequestTrigger.TYPE].pr;
