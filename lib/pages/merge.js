...
module.exports = React.createClass({displayName: 'MergePage',
    ...
    render: function() {
        ...
        if (!mergeable) {
            return (
                <div>PR is not mergeable; try merging {pr.getDest().getRef()} out into {pr.getSource().getRef()}.</div>
            );
        }
        return (
            <div>
                <a onClick={this.squashMergeWithRewrite}>Squash rewriting history</a><br />
                <a onClick={this.squashMergeWithoutRewrite}>Squash leaving PR closed not merged</a><br />
