...
module.exports = React.createClass({displayName: 'ListPage',
    ...
    loadPulls: function() {
        var self = this;
        self.props.controller.httpApi.getUser()
            .done(function(userData) {
                self.props.controller.httpApi.getOpenPullRequests(userData.body.login, "author")
                .then(self._fillComments)
                .then(self._loadMyPulls)
                .catch(function(err) {
                    console.error(JSON.stringify(err));
                    self.setState({
                        error: "Unexpected error loading pull requests"
                    });
                });

                self.props.controller.httpApi.getOpenPullRequests(userData.body.login, "assignee")
                .then(self._fillComments)
