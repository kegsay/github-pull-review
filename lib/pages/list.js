...
module.exports = React.createClass({displayName: 'ListPage',
    ...
    loadPulls: function() {
        var self = this;
        this.state.pullsState = loading;
        this.state.assignedPullsState = loading;
        var self = this;
    ...
    render: function() {
        ...
        if (this.state.pullsState === loaded && this.state.assignedPullsState === loaded) {
            ...
            var owned = [].concat(
                ...
                this._renderPRs("Yours, LGTM:", this.state.pulls[lgtmedKey]),
                this._renderPRs("Unassigned:", this.state.pulls[unassignedKey]),
                this._renderPRs("Yours, awaiting review:", this.state.pulls[awaitingReviewKey])
            );
            if (owned.length === 0) {
                owned = "None";
            }
    ...
    _renderPRs: function(heading, pulls, display) {
        ...
        return [(<h2>{heading}</h2>), (<ul>{pulls.map(function(pr) {
            ...
            return <li><Link to={`/repos/${pr.getRepo()}/${pr.getId()}`}>{pr.getId()}: {pr.getTitle()}</Link> - <a href={repoUrl} target="_blank">{pr.getRepo()}</a> {who}</li>
        })}</ul>)];
    }
});
