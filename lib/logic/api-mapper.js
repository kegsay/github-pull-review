...
function toCommit(obj) {
    ...
    if (!obj.author) {
        author = {
            ...
            avatar_url: (
                "https://assets-cdn.github.com/images/gravatars/gravatar-user-420.png?ssl=1"
            ),
            html_url: null
        };
    }
...
module.exports.getCommitsFromGithubApi = function(apiCommits, apiData) {
    ...
    // .commit.comment_count => number of commit comments
...
module.exports.getCommitLineCommentsFromGithubApi = function(apiCommitLineComments) {
    // apiCommitLineComments is an array of objects (age order, last = newest) with
    // keys similar to but not quite like getLineComments. Notably:
    // .id => Line comment ID
    // .html_url => HTML url for this comment
    // .path => Path from root to the file being commented on (0-len string if comment on commit)
    // .user.[login/avatar_url] => the github user making the comment
    // .body => Comment body
    // .commit_id => The commit the comment was made on
    // .position => The line index in the diff where the comment was made. (0 indexed) (null if on commit)
    return []; // TODO
}
