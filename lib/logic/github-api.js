...
GithubApi.prototype.getEmailAddresses = function() {
    return this._get("/user/emails").then((res) => res.body);
}
...
GithubApi.prototype.squashMerge = function(
        sourceRepo, sourceRef, destRepo, destRef, commitMessage, email) {
    return this.getUser().then((userData) => {
        var data = {
            ...
            password: this.token,
            committer_name: userData.body.name || userData.body.login,
...
GithubApi.prototype.squashBranch = function(
        repo, ref, baselineRepo, baselineRef, commitMessage, email) {
    return this.getUser().then((userData) => {
        var data = {
            repo: repo,
            branch: ref,
...
GithubApi.prototype.authenticate = function(scopes) {
    var flattenedScopes = scopes.join(",");
    var redirectUri = `${window.location.protocol}//${window.location.host}` +
        `${window.location.pathname}`;
    window.location = `${this.supplementaryUrl}/oauth/authorize` +
        `?scopes=${flattenedScopes}&redirect_uri=${redirectUri}`;
};

GithubApi.prototype.followLink = function(url) {
