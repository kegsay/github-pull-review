...
Controller.prototype._refreshDiffs = function(pr) {
    this.dispatcher.dispatch(triggers.create("get_diffs", {
        repo: pr.getRepo(),
        id: pr.getId(),
        allow_cached: false
    }));
...
Controller.prototype._handleMergeError = function(err) {
    console.log("Got merge error: %s", JSON.stringify(err));
    this.dispatcher.dispatch(triggers.create("merge_error", {"error": "Error merging"}))
};
