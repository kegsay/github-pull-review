...
Controller.prototype.retrievePullRequest = function(repo, pr, dispatch) {
    ...
    return Promise.try(function() {
        ...
        var commitsWithComments = apiCommits.body.filter(function (c) {
            return c.commit.comment_count > 0;
        });

        var promises = [apiData, apiComments, apiCommits];

        if (commitsWithComments.length > 0) {
            console.log("%s commits have commit comments", commitsWithComments.length);
            // add more args to be re-spread. We abuse ES6 rest params since we don't
            // actually know at compile time how many args the next spread will have...
            /*
            commitsWithComments.forEach(function(c) {
                promises.push(self.httpApi.followLink(c.comments_url));
            }); */
        }
        return promises;
    }).spread(function(apiData, apiComments, apiCommits, ...apiCommitComments) {
        ...
        var commitLineComments = [];
        if (apiCommitComments && apiCommitComments.length > 0) {
            commitLineComments = apiMapper.getCommitLineCommentsFromGithubApi(
                apiCommitComments
            );
        }
        ...
        pullRequest.setCommitLineComments(commitLineComments);
        if (body.merged) {
