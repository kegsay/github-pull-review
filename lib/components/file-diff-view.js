...
module.exports = React.createClass({displayName: 'FileDiffView',
    ...
    render: function() {
        ...
        if (this.props.comments.length > 0) {
            var headCommentCount = this.props.comments.filter(function(c) {
                return c.getSha() === pr.getHeadSha();
            }).length;

            commentCount = (
                <span className="FileDiffView_counts_comments">
                    {headCommentCount} comments ({this.props.comments.length} total)
                </span>
            );

            if (headCommentCount < this.props.comments.length) {
                otherCommentsButton = (
